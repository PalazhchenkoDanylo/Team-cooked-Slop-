...
class RFIDReader(threading.Thread):
    ...
    def _read_loop(self, device):
        while not self.stop_event.is_set():
            ...
            # Clean the tag of whitespace and newline breaks
            tag = raw.decode(errors="ignore").strip()
            
            # Hardware noise filter: ignore empty strings or tiny glitch tags
            if len(tag) < 4:
                continue
    ...
    def _publish(self, tag):
        ...
        # GLOBAL DEBOUNCE: Ignore ALL incoming scans for 2 seconds after any scan.
        # This completely destroys "ghost" scans where the reader sends a valid ID
        # followed immediately by a corrupted, unrecognized ID as you pull the card away.
            ...
            try:
                ...
                if keys.accepts(tag):
                    ...
                    if active_user is None:
                        ...
                        last_event = f"login: traffic enabled for {user_name}"
                        log.add(last_event)
                    elif active_user == user_name:
                        gateway.disable()
                        db_log_out(active_user)
                        active_user = None
                        last_event = f"logout: traffic disabled for {user_name}"
                        log.add(last_event)
                    else:
